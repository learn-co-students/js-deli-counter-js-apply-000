...
function nowServing(katzDeliLine) {
  if (katzDeliLine.length === 0) {  // (!katzDeliLine.length) ? can we go over the ! Not logical operator?
...
function currentLine(katzDeliLine) {
  ...
  if (katzDeliLine.length === 0) {  // (!katzDeliLine.length) ? can we go over the ! Not logical operator?
    ...
    for (var i=0; i<katzDeliLine.length; i++) {  // iteration using a for loop for new array
